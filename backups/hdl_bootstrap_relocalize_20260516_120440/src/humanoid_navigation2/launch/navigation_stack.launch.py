...
2. 定位层：定位节点（发布map→odom TF）+ map_server（发布2D栅格地图）
   ...
   当前实验版：方案C只做启动全局重定位，桥接到方案A的/initialpose，
   后续由方案A发布map→odom并持续定位。
...
def generate_launch_description():
    ...
    # ┌──────────────────────────────────────────────────────────────────────────┐
    # │  方案C-bootstrap + 方案A-tracking 实验方案                                │
    # │                                                                          │
    # │  C 只做启动全局重定位，不发布 map->odom TF；桥接节点把 C 的 /hdl_bootstrap/odom
    # │  换算成 A 需要的 /initialpose。之后由 A 独占发布 map->odom 并持续定位。     │
    # └──────────────────────────────────────────────────────────────────────────┘
        ...
        composable_node_descriptions=[
            ...
            ComposableNode(
                ...
                parameters=[{
                    ...
                    'global_localization_ambiguous_max_fitness_score': 0.03,
    ...
    #             parameters=[{
                    ...
                    # # 连续一致性队列。当前正确簇约 0.004~0.018，错误边缘簇约 0.049。
                    # 'global_localization_ambiguous_max_fitness_score': 0.03,
    ...
    return LaunchDescription([
        ...
        *fastdds_env_setup,
        ...
        # ┌─ 方案C-bootstrap：只用于启动全局重定位，不发布 map->odom TF ─┐
        ...
        # ┌─ 方案A：单分辨率 NDT 定位，等待 C bootstrap 发布 /initialpose ─┐
