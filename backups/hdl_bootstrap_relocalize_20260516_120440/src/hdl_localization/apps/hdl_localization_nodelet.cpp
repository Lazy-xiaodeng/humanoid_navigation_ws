...
private:
  ...
  bool perform_relocalize(bool allow_overwrite_existing_pose, bool use_recovery_prior = false) {
    ...
    if (validate_global_localization_with_scan_matching) {
      ...
      if (use_recovery_prior && have_last_accepted_pose) {
        ...
        accepted_candidates.erase(
          std::remove_if(
            accepted_candidates.begin(),
            accepted_candidates.end(),
            [this, &prior_pose](const auto& candidate) {
              ...
              RCLCPP_INFO_STREAM(
                get_logger(),
                "discard recovery candidate[" << candidate.index
                  << "] far from last accepted pose, pose=("
        ...
        if (accepted_candidates.empty()) {
          RCLCPP_WARN(get_logger(), "reject recovery global localization result: no candidate is close to the last accepted pose");
          relocalizing = false;
          return false;
        }
      }
        ...
        if (margin < global_localization_min_fitness_margin) {
          ...
          if (
            ...
            RCLCPP_WARN_STREAM(
              ...
              "reject global localization result: ambiguous fitness margin "
                << margin << " < " << global_localization_min_fitness_margin
                << " and best fitness " << best.fitness
                << " > confident threshold "
                << global_localization_ambiguous_max_fitness_score);
            relocalizing = false;
            return false;
          }
          ...
          RCLCPP_WARN_STREAM(
            ...
            "global localization result has ambiguous fitness margin "
              << margin << " < " << global_localization_min_fitness_margin
              << " but best fitness is confident; deferring acceptance to consistency gate "
              << required_consistent_results << " consecutive results");
        }
      }
