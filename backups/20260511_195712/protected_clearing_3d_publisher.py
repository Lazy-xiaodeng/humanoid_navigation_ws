...
OCCUPIED_MIN_RANGE = 0.20
OCCUPIED_MAX_RANGE = 5.0
OCCUPIED_MIN_Z = 0.10
OCCUPIED_MAX_Z = 1.70
OCCUPIED_ANGLE_PADDING_BINS = 15
OCCUPANCY_TIMEOUT_SEC = 1.0
...
class ProtectedClearing3DPublisher(Node):
        ...
        for point in pc2.read_points(cloud_msg, field_names=("x", "y", "z"), skip_nans=True):
            ...
            for offset in range(-OCCUPIED_ANGLE_PADDING_BINS, OCCUPIED_ANGLE_PADDING_BINS + 1):
                ...
                if 0 <= padded_index < self.bin_count:
                    occupied.add(padded_index)

        self.occupied_bins = occupied
        self.last_occupancy_time = time.time()

    def build_clearing_points(self, blocked_bins):
        points = []
        for bin_index, angle in enumerate(self.bin_angles):
            if bin_index in blocked_bins:
                continue
    ...
    def heartbeat_callback(self, _cloud_msg):
        ...
        blocked_bins = set()
        if time.time() - self.last_occupancy_time <= OCCUPANCY_TIMEOUT_SEC:
            blocked_bins = self.occupied_bins

        clearing_points = self.build_clearing_points(blocked_bins)
