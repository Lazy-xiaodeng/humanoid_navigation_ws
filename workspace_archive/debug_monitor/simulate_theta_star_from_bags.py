...
def path_len(points):
    return sum(d2(a, b) for a, b in zip(points, points[1:])) if len(points) > 1 else 0.0
...
class Grid:
    ...
    def world_to_map(self, p):
        x = int((p[0] - self.ox) / self.res)
        y = int((p[1] - self.oy) / self.res)
        return x, y
    ...
    def safe(self, c, allow_unknown=False):
...
def los_cost(grid, a, b, w_traversal, allow_unknown):
    ...
    for c in bresenham(a, b):
        if not grid.safe(c, allow_unknown):
            return None
        raw = grid.cost(c)
        if raw < 0:
            raw = 100
...
def theta_star(grid, start_w, goal_w, w_euc=1.0, w_traversal=2.0, allow_unknown=False):
    ...
    if not grid.safe(start, allow_unknown) or not grid.safe(goal, allow_unknown):
        return []
    ...
    parent = {start: start}
    closed = set()
    open_heap = [(d2(start, goal), 0, start)]
    seq = 1
    ...
    while open_heap:
        ...
        for dx, dy in moves:
            ...
            if not grid.safe(nb, allow_unknown) or nb in closed:
                continue
            ...
            if sl_cost is not None:
                cand_parent = p
                cand_g = g[p] + w_euc * d2(p, nb) + sl_cost
            else:
                step_cost = los_cost(grid, current, nb, w_traversal, allow_unknown)
                if step_cost is None:
                    continue
                cand_parent = current
                cand_g = g[current] + w_euc * d2(current, nb) + step_cost
    ...
    # Match Nav2-style dense paths enough for visual comparison.
    points = []
    for a, b in zip(raw, raw[1:]):
        aw = grid.map_to_world(a)
        bw = grid.map_to_world(b)
        steps = max(1, int(d2(aw, bw) / grid.res))
        for i in range(steps):
            t = i / steps
            points.append((aw[0] + (bw[0] - aw[0]) * t, aw[1] + (bw[1] - aw[1]) * t))
    points.append(grid.map_to_world(raw[-1]))
    if points:
        points[0] = start_w
        points[-1] = goal_w
    return points
...
def analyze_bag(bag_dir, ids_filter):
        ...
        if topic == "/global_costmap/costmap":
            ...
            for item in list(pending):
                ...
                if len(original) < 2 or last_grid is None:
                    pending.remove(item)
                    continue
                theta = theta_star(last_grid, original[0], original[-1])
                rows.append({
...
def render_html(rows, output):
    ...
    parts = [
        "<!doctype html><html><head><meta charset='utf-8'><title>ThetaStar bag simulation</title>",
        "<style>body{font-family:Arial,sans-serif;margin:24px;color:#222}svg{border:1px solid #ccc;background:#fafafa;margin:12px 0 28px}table{border-collapse:collapse;margin:10px 0 28px}td,th{border:1px solid #ddd;padding:4px 8px;font-size:13px}.orig{color:#d62728}.theta{color:#1f77b4}.straight{color:#999}</style>",
        "</head><body><h1>ThetaStar bag simulation</h1>",
        "<p><span class='orig'>red=bag /plan</span>, <span class='theta'>blue=offline ThetaStar</span>, <span class='straight'>gray=straight start-goal</span>.</p>",
    ]
    ...
    for bag, bag_rows in grouped.items():
        ...
        canvas_w = int(draw_w + 2 * margin)
        canvas_h = int(draw_h + 2 * margin)
        ...
        parts.append("<table><tr><th>ids</th><th>orig_len</th><th>orig_dev</th><th>theta_len</th><th>theta_dev</th><th>improve_dev</th></tr>")
        for r in bag_rows:
            olen = path_len(r["original"])
            odev = max_deviation(r["original"])
            tlen = path_len(r["theta"])
            tdev = max_deviation(r["theta"])
            parts.append(
                f"<tr><td>{html.escape(','.join(r['ids']))}</td><td>{olen:.2f}</td><td>{odev:.2f}</td>"
                f"<td>{tlen:.2f}</td><td>{tdev:.2f}</td><td>{odev - tdev:.2f}</td></tr>"
            )
        parts.append("</table>")
        parts.append(f"<svg width='{canvas_w}' height='{canvas_h}' viewBox='0 0 {canvas_w} {canvas_h}'>")
        for r in bag_rows:
            label = ",".join(r["ids"])
            parts.append(f"<polyline points='{polyline([r['start'], r['goal']], sx, sy)}' fill='none' stroke='#999' stroke-width='1' stroke-dasharray='5,5'/>")
            parts.append(f"<polyline points='{polyline(r['original'], sx, sy)}' fill='none' stroke='#d62728' stroke-width='2' opacity='0.65'/>")
            if r["theta"]:
                parts.append(f"<polyline points='{polyline(r['theta'], sx, sy)}' fill='none' stroke='#1f77b4' stroke-width='2' opacity='0.8'/>")
...
def main():
    ...
    parser.add_argument("--output", default="workspace_archive/debug_monitor/theta_star_bag_simulation.html")
    args = parser.parse_args()

    rows = []
    for bag in args.bags:
        rows.extend(analyze_bag(Path(bag), set(args.ids)))
    ...
    print("bag,ids,orig_len,orig_dev,theta_len,theta_dev")
    for r in rows:
        print(
            f"{r['bag']},{'+'.join(r['ids'])},{path_len(r['original']):.3f},"
            f"{max_deviation(r['original']):.3f},{path_len(r['theta']):.3f},"
            f"{max_deviation(r['theta']):.3f}"
        )
