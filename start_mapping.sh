...
BAG_ROOT="${BAG_ROOT:-$HOME/fast-lio-bags}"
...
finish_mapping() {
  ...
  log "[1/7] Stopping recorders and flushing bag/PCD data..."
  ...
  log "[2/7] Saving 2D occupancy grid map: ${MAP_PREFIX}.yaml/.pgm"
  save_2d_occupancy_grid_map || true

  log "[3/7] Saving slam_toolbox posegraph: ${MAP_PREFIX}.posegraph/.data"
  ...
  log "[4/7] Generating derived PCD files..."
  if [ -s "$PCD_FILE" ]; then
  ...
  log "[5/7] Stopping mapping launch..."
  stop_process_int "mapping launch" "$MAPPING_PID" 30

  log "[6/7] Checking output files..."
  local failed=0
  ...
  if [ "$failed" -eq 0 ]; then
    log "[7/7] Registering map for APP map switching..."
    register_completed_map
...
setsid ros2 bag record -o "$BAG_DIR" \
  ...
  /prior_localization/robosense_input_odom \
...
log "The script will then save maps, bag, PCD files, Open3D/RoboSense prior maps, and check output files."
