...
bool LidarMatcherCeres::align(const PointCloudT::Ptr &source_cloud,
  ...
  if (debug_print_) {
    std::cout << std::fixed << lidar_time << "," << iter_cnts << "," << ceres_opt_times << ","
              << result_pose.xyz.transpose() << "," << result_pose.q.coeffs().transpose() << std::endl;
  }
  iter_cnts = 0;
  ceres_opt_times = 0.0;
...
bool LidarMatcherEigen::align(const PointCloudT::Ptr &source_cloud,
  ...
  if (debug_print_) {
    std::cout << std::fixed << lidar_time << "," << iter_cnts << "," << eigen_opt_times << ","
              << result_pose.xyz.transpose() << "," << result_pose.q.coeffs().transpose() << std::endl;
  }
  iter_cnts = 0;
  eigen_opt_times = 0.0;
