...
`test/probes/*_probe.cpp` 是协议一致性和模块质量验证时使用的开发期小程序，默认不参与正式构建和安装。需要单独验证某个纯逻辑模块时，可临时执行：
...
- 保持 APP 协议字段兼容，尤其是 `system_status`、`navigation_status`、`waypoints_data` 和动作/表情列表。
