...
private:
  void configure_data_store_ttl()
  {
    ...
    data_store_.set_expiry("navigation_command_result", config_.navigation_status_ttl_sec);
  ...
  void navigation_ack_callback(const std_msgs::msg::String & msg)
  {
    ...
    data_store_.set_data("navigation_command_result", built.json, timestamp);
  ...
  void publish_navigation_ack_push(
    ...
    const double timestamp)
  {
    // 导航 ack 对 APP 仍使用旧 Test 链路的 navigation_command_result data_type。
    // navigation_ack_callback() 同时更新 navigation_status 缓存，保证导航页查询也能拿到最近 ack 上下文。
