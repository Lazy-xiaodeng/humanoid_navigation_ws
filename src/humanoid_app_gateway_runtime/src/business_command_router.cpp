  ...
  const double timestamp_sec) const
{
  ...
  if (command_type == "waypoint_management") {
    ...
    if (inner_command == "set_broadcast_volume") {
      ...
      add_string(payload, allocator, "command_type", inner_command);
      // Test 工作区的单地图导航链路仍然使用 waypoint_id / waypoint_ids /
      // exhibition_ids 三类字段，下游 humanoid_route_runtime 会直接读取这些目标。
      // 这里保留 Todesk 路线任务字段的同时补齐 Test 字段，避免 APP 命令被转成空目标。
      add_string(payload, allocator, "waypoint_id", value_to_string(find_member(command_data, "waypoint_id")));
      add_copy_or_empty_array(payload, allocator, "waypoint_ids", find_member(command_data, "waypoint_ids"));
      add_copy_or_empty_array(payload, allocator, "exhibition_ids", find_member(command_data, "exhibition_ids"));
