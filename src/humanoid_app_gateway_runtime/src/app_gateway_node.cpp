...
private:
  ...
  void client_read_loop(const std::shared_ptr<ClientConnection> & client)
  {
    try {
      while (running_) {
        ...
        RCLCPP_INFO(
          get_logger(),
          "APP WebSocket 收到客户端消息：client=%s bytes=%zu",
          client->id.c_str(),
          message.size());
      ...
      RCLCPP_WARN(
        get_logger(),
        "APP WebSocket 客户端读循环结束：client=%s reason=%s",
        client->id.c_str(),
        ex.what());
  ...
  void handle_business_command(const std::string & message, const std::string & client_id)
  {
    ...
    RCLCPP_INFO(
      get_logger(),
      "APP 业务命令路由成功：client=%s target=%d payload_bytes=%zu ack_bytes=%zu",
      client_id.c_str(),
      static_cast<int>(routed.target),
      routed.payload_json.size(),
      routed.ack_json.size());
  ...
  // Test 工作区当前没有播报音量服务接口；保留命令识别，返回明确错误，避免影响其他导航命令。
  void handle_broadcast_volume_service(const std::string & payload_json)
  {
    ...
    publish_navigation_ack(build_broadcast_volume_ack(
      request_id,
      "error",
      "broadcast_volume_unavailable",
      "当前 Test 工作区未接入播报音量服务"));
  }
