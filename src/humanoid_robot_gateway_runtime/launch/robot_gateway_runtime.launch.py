...
速度发送、动作执行和动作库同步仍默认关闭，避免单独启动时意外控制机器人。
...
def generate_launch_description():
    ...
    return LaunchDescription([
        ...
        DeclareLaunchArgument(
            ...
            default_value='false',
            description='是否允许把 /cmd_vel 真实发送给机器人；默认 false',
        ),
        DeclareLaunchArgument(
            'motion_execution_enable',
            default_value='false',
            description='是否允许真实执行 APP 上半身动作；默认 false',
        ),
        DeclareLaunchArgument(
            'motion_allow_enter_menu',
            default_value='false',
            description='执行动作前是否允许自动切换 Menu；默认 false',
        ),
        DeclareLaunchArgument(
            'motion_allow_return_walk',
            default_value='false',
            description='动作结束后是否允许自动切回 Walk；默认 false',
        ),
        DeclareLaunchArgument(
            'gesture_sync_enable',
            default_value='false',
            description='是否连接成功后拉取机器人动作库并写入 gestures.yaml；默认 false',
        ),
        Node(
