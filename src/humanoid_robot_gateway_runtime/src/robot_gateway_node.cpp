...
std::string execute_motion_json(const std::string & motion_name)
{
  ...
  document.AddMember(
    "motion_name",
    rapidjson::Value(motion_name.c_str(), allocator).Move(),
    allocator);
  return rapidjson_value_to_json(document);
}
...
private:
  ...
  void execute_motion_task(const MotionStartDecision decision)
  {
    ...
    try {
      ...
      } else if (wait_for_motion_notify(decision.motion_timeout_sec, &notify_json, &result)) {
        if (result.empty() || result == "success") {
    ...
    // 旧链路的 Python finally 语义是：只要进入过 Menu，动作成功、失败或超时后都尽量切回 Walk。
    // 这里保持同样行为，避免异常早退时机器人停留在动作库模式导致后续导航速度被拦截。
    if (entered_menu) {
      std::this_thread::sleep_for(std::chrono::milliseconds(500));
    }
    if (config_.motion_allow_return_walk && entered_menu && !is_walk_state(current_robot_state())) {
  ...
  {
    // 优先使用 YAML 配置的动作库路径；未配置时定位表情/动作资源运行包内默认路径。
