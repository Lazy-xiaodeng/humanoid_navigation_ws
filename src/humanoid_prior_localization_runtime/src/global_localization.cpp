 ...
 *
 * 文件作用：
 * 1. 提供 OP/Open3D 先验地图定位节点 global_localization_node。
 * 2. 订阅标准轴局部里程计和注册点云，将当前点云与先验 PCD 地图做 Open3D 配准。
 * 3. 输出全局定位候选、定位置信度和调试点云，由 prior_map_odom_bridge_cpp 统一转换成 map->odom。
 *
 * 上游节点：
 * - fastlio_open3d_axis_adapter：发布 /prior_localization/open3d_input_odom 与 /prior_localization/open3d_input_cloud。
 * - Fast-LIO/RoboSense 点云链路：提供原始 odom 和注册点云。
 *
 * 下游节点：
 * - prior_map_odom_bridge_cpp：订阅 /prior_localization/odom 与 /prior_localization/confidence。
 * - RViz/调试工具：可订阅 open3d_map、submap、scan、scan2map 等调试输出。
 *
 * 维护说明：
 * - 本文件属于先验地图定位核心算法，迁移包名时不改变算法语义。
 * - map->odom TF 不在本节点发布，避免和桥接节点抢占同一条全局 TF。
 */
...
private:
    ...
    /// @brief 是否直接使用输入 Odometry 的 pose。
    /// 原仓库默认把输入当作 imu_link，再通过 TF 换到 base_link。
    /// 本工作区前面已经用 fastlio_open3d_axis_adapter 发布了标准轴
    /// odom->prior_open3d_base，因此这里默认直接使用输入 pose，避免重复套用外参。
    bool use_input_odom_pose_directly_ = true;
    /// @brief 是否由先验地图定位节点直接发布 TF。
    /// 本工作区中 map->odom 由 humanoid_localization_runtime/prior_map_odom_bridge_cpp 统一维护，
    /// 因此默认关闭，避免两个节点同时发布 map->odom 造成 TF 冲突。
                                             ...
                                             tf_listener_(std::make_shared<tf2_ros::TransformListener>(tf_buffer_))
{
    ...
    this->declare_parameter<bool>("use_input_odom_pose_directly", true);
    ...
    this->get_parameter("use_input_odom_pose_directly", use_input_odom_pose_directly_);
    ...
    RCLCPP_INFO(this->get_logger(), "  use_input_odom_pose_directly: %s", use_input_odom_pose_directly_ ? "true" : "false");
...
bool GloabalLocalization::GetTfTransformToMatrix(std::string frame_id, std::string child_frame_id, Eigen::Matrix4d &matrix)
{
    ...
    catch (tf2::TransformException &e)
    {
        RCLCPP_WARN(
            this->get_logger(),
            "[GetTransformMatrix]: %s, use identity transform for %s -> %s",
            e.what(),
            frame_id.c_str(),
...
void GloabalLocalization::CallbackBaselink2Odom(const nav_msgs::msg::Odometry::SharedPtr baselink2odom)
{
    ...
    if (use_input_odom_pose_directly_)
    {
        // 本系统通过 fastlio_open3d_axis_adapter 已经发布标准轴
        // odom->prior_open3d_base。这里直接使用输入 pose，避免原仓库
        // 再把 imu_link/base_link 外参套一遍，造成位姿偏移。
        mat_baselink2odom_ = mat_imulink2odom;
    }
    else
    {
