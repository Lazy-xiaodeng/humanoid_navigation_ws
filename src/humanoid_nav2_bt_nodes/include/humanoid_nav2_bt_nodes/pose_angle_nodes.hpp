...
#define HUMANOID_BT_HAS_JSON_EXPORT 0
#else
#error "BehaviorTree.CPP action_node.h not found"
#endif

#if HUMANOID_BT_HAS_JSON_EXPORT && __has_include("behaviortree_cpp/json_export.h")
#include "behaviortree_cpp/json_export.h"
#define HUMANOID_BT_REGISTER_JSON(Type) BT::RegisterJsonDefinition<Type>()
#else
#define HUMANOID_BT_REGISTER_JSON(Type) do {} while (false)
#endif
#include "geometry_msgs/msg/pose_stamped.hpp"
#if __has_include("nav2_behavior_tree/json_utils.hpp")
...
struct SpinResultTraits
{
  using ErrorCode = uint16_t;

  static constexpr ErrorCode NONE = 0;
  static constexpr ErrorCode UNKNOWN = 1;
  static constexpr ErrorCode TIMEOUT = 2;
  static constexpr ErrorCode TF_ERROR = 3;

  template<typename ResultPtrT>
  static ErrorCode errorCode(const ResultPtrT & result)
  {
    return result ? NONE : UNKNOWN;
  }
};

template<typename ResultT>
struct SpinResultTraits<ResultT, std::void_t<typename ResultT::_error_code_type>>
{
  using ErrorCode = typename ResultT::_error_code_type;

  static constexpr ErrorCode NONE = ResultT::NONE;
  static constexpr ErrorCode UNKNOWN = ResultT::UNKNOWN;
  static constexpr ErrorCode TIMEOUT = ResultT::TIMEOUT;
  static constexpr ErrorCode TF_ERROR = ResultT::TF_ERROR;

  template<typename ResultPtrT>
  static ErrorCode errorCode(const ResultPtrT & result)
  {
    return result ? result->error_code : UNKNOWN;
  }
};
...
public:
  ...
  {
    HUMANOID_BT_REGISTER_JSON(geometry_msgs::msg::PoseStamped);
...
public:
  ...
  using ResultTraits = SpinResultTraits<Action::Result>;
  using ErrorCode = ResultTraits::ErrorCode;
