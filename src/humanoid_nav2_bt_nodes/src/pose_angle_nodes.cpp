...
BT::NodeStatus SpinToPose::onRunning()
{
  ...
  if (phase_ == Phase::WAITING_FOR_GOAL_HANDLE) {
    if (isFutureReady(future_goal_handle_)) {
      ...
      if (!goal_handle_) {
        ...
        setErrorCode(Action::Result::UNKNOWN);
        resetActionState();
        return BT::NodeStatus::FAILURE;
      }
    ...
    if (std::chrono::steady_clock::now() - goal_sent_steady_time_ > server_timeout_)
    {
      ...
      setErrorCode(Action::Result::TIMEOUT);
      resetActionState();
      return BT::NodeStatus::FAILURE;
    }
  ...
  if (phase_ == Phase::WAITING_FOR_RESULT) {
    ...
    const auto error_code = result.result ? result.result->error_code : Action::Result::UNKNOWN;
    setErrorCode(error_code);

    if (result.code != rclcpp_action::ResultCode::SUCCEEDED ||
      error_code != Action::Result::NONE)
    {
      RCLCPP_WARN(
        node_->get_logger(),
...
BT::NodeStatus SpinToPose::sendSpinIfNeeded()
{
  try {
    ...
    if (check.aligned) {
      setErrorCode(Action::Result::NONE);
      return BT::NodeStatus::SUCCESS;
    }
    ...
    setErrorCode(Action::Result::TF_ERROR);
  } catch (const std::exception & ex) {
...
void SpinToPose::setErrorCode(Action::Result::_error_code_type error_code)
{
