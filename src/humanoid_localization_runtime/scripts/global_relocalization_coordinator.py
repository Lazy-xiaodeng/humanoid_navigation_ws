#!/usr/bin/env python3
"""Coordinate on-demand global relocalization and guarded localization handoff."""
...
class GlobalRelocalizationCoordinator(Node):
    """Own one recovery attempt and apply only a correlated, current-map result."""
    ...
    def __init__(self):
        ...
        self.auto_apply = bool(self.declare_parameter("auto_apply", False).value)
    ...
    def on_trust(self, msg: String):
        ...
        if self.state == "verifying" and trusted:
            ...
            return
        if (
            self.auto_trigger and not self.auto_trigger_suppressed
    ...
    def process_accepted_status(self, payload: Dict[str, Any]):
        ...
        if self.dry_run or not self.auto_apply:
            self.publish_event(
                "localization_relocalize_attempt_deferred", "dry_run_no_pose_application",
                result_code="dry_run"
            )
            return
    ...
    def apply_candidate(self, pose: PoseStamped, map_odom: PoseStamped):
        self.bridge_apply_pub.publish(map_odom)
        ...
        self.publish_event("localization_initialpose_published", "bridge_and_ro_pose_published")
    ...
    def on_navigation_status(self, msg: String):
        ...
        if event_type in ("navigation_pending", "waypoint_started"):
            ...
            if self.state == "failed":
                self.state = "idle"
                self.attempt_number = 0
            if self.auto_trigger and self.recovery_required() and self.state in ("idle", "waiting_map"):
                self.start_attempt()
    ...
    def on_timer(self):
        ...
        elif self.state == "applying" and now - self.apply_started > self.apply_timeout_sec:
            self.schedule_retry_or_fail("bridge_apply_timeout")
