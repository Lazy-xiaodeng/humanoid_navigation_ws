...
def generate_launch_description():
    ...
    pkg_roi_obstacle = get_package_share_directory('humanoid_obstacle_runtime')
    pkg_prior_localization = get_package_share_directory('humanoid_prior_localization_runtime')
    ...
    default_roi_obstacle_params_file = os.path.join(pkg_roi_obstacle, 'config', 'obstacle_runtime.yaml')
    ...
    def localization_runtime_node(executable, node_name, extra_parameters=None):
        parameters = [
            {
    ...
    robot_realpose_publisher = localization_runtime_node(
    ...
    prior_map_localization_node = Node(
        ...
        parameters=[
            # Open3D 定位包自带默认参数。
            os.path.join(pkg_prior_localization, 'config', 'loc_param_g1.yaml'),
            {
                # 时间源。
                'use_sim_time': use_sim_time,
    ...
    prior_map_odom_bridge_node = localization_runtime_node(
    ...
    localization_ready_gate = localization_runtime_node(
        'wait_for_tf',
