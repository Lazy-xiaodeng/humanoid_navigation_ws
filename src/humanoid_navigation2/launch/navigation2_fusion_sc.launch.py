...
def generate_launch_description():
    ...
    localization_odom_fusion_node = nav2_python_node(
        ...
        'localization_odom_fusion',
        {
            ...
            'init_timeout_sec': 20.0,               # INITIALIZING超时，超时后主动请求SC recovery
            # LOST recovery 只做软验收: 不用 yaw 硬拒绝，只拦截明显离谱的 XY 跳变
            ...
            'lock_recovery_max_correction_m': 0.3,       # 恢复时允许的最大 correction(m)
            'lock_early_lost_rejection_rate': 0.9,       # 锁定期内拒绝率超此值→提前LOST
            'lock_early_lost_min_frames': 30,            # 锁定期内最少累积帧数后允许提前LOST
            'recovery_pose_jump_max_m': 5.0,             # 恢复时 frozen→NDT 跳变上限(m)
