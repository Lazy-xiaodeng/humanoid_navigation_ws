...
5. 当前方连续多帧 clear 后，ROS 自动恢复当前导航。
6. 恢复时推送 `navigation_resumed`，其中 `resume_source=obstacle_wait`。

注意：这条自动恢复链路不依赖 APP 的“继续”按钮。APP 后续如果支持人工脱困后点击继续，可复用现有 `resume_navigation` 命令。

## 2. Websocket 外层格式

APP 收到的导航事件由数据整合节点统一包装为 websocket push。APP 做 UI 状态判断时，建议优先使用 `data.event_type` 和 `data.event_data`。
...
触发时机：导航执行中，机器人持续停滞超过 `obstacle_block_timeout`，当前默认 `2s`，ROS 认为前方路径被障碍物挡住。
...
ROS 通过 local costmap 前方窗口判断障碍消失，并且连续 clear 达到 `clear_required_frames`，默认 5 帧后，自动恢复导航。
...
因此 `reason` 字段目前未必会透传到 `navigation_state_manager`。如果 APP 需要 ROS 精确区分“人工脱困完成后继续”，建议后续同步扩展 websocket server 的 `navigation_control` 透传字段。
...
| 字段 | 类型 | 说明 |
|---|---:|---|
| `pause_source` | string | 暂停来源：`user_request`、`obstacle_wait`、`prior_map_degraded` |
...
4. 当前 `reason` 透传到 ROS 的命令链路还未完全扩展，后续如需精细区分继续原因，需要同步改 websocket server。
