/*
 * roi_obstacle_detector_cpp.cpp
 *
 * 文件作用：
 * 1. 订阅点云并转换到机器人 base_footprint 坐标系。
 * 2. 对前方 ROI 区域进行地面补偿、体素降采样、点数统计和可选聚类。
 * 3. 输出前方是否存在障碍物，用于导航暂停后的恢复校验和障碍状态调试。
 *
 * 上游：原始雷达点云或滤波点云，可选 IMU 姿态输入。
 * 下游：/front_obstacle/has_obstacle、/front_obstacle/debug、/front_obstacle/roi_cloud。
 */
...
private:
  ...
  std::vector<Point> read_xyz(const sensor_msgs::msg::PointCloud2 & msg)
  {
