...
`kindr` 是外部机器人运动学/动力学数学库，提供姿态、旋转、变换、四元数和相关几何工具。

它不是本项目自研业务包，也不启动 ROS 节点；它作为底层 C++ 数学依赖被其他库或算法使用。
...
- 需要 kindr 数学类型或变换工具的 C++ 包。
- 工作区中的 `kindr_ros` 或其他第三方包可能依赖它。

## 使用方式

业务包如果需要使用 kindr，需要在 CMake 中查找并链接：
...
- 这是第三方库，不建议随意修改源码。
- 如果后续确认没有任何包依赖它，可以再灰度 `COLCON_IGNORE` 验证是否可删除。
- 如果升级版本，需要同步验证依赖它的包是否存在 API 兼容问题。
