...
class UnifiedDataIntegrationNode(Node):
    ...
    def publish_navigation_status_event(self, status_data: Dict[str, Any]):
        ...
        try:
            ...
            push_msg["data"] = status_data
            push_msg["metadata"].update({
                "push_reason": "navigation_event",
                "data_freshness": time.time() - self.last_update_times.get("navigation_status", time.time()),
                "qos_level": "realtime"
            })

            self.publish_push_message(push_msg)
            self.get_logger().info(
                f"📤 立即推送导航事件: {status_data.get('event_type', 'unknown')}"
            )
        except Exception as e:
    ...
    def should_push_navigation_status_immediately(event_type: str) -> bool:
        """只立即推送离散事件；高频 progress_update 继续走原有周期推送。"""
