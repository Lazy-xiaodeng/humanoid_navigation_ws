 ...
 *
 * 文件作用：
 *   1. 这是 vendored KOKIAOKI/3d_bbs 的 CPU 版 3D-BBS 搜索实现。
 *   2. 负责根据多分辨率 voxel map 对离散平移/姿态分支评分，并用 priority queue 做 branch-and-bound。
 *   3. 本包在上游实现基础上增加 top-K leaf 候选保存，便于验证阶段分析相似位置歧义。
 *
 * 修改边界：
 *   - 原始评分、分支展开、超时逻辑保持不变。
 *   - 新增 top_transforms 队列和剪枝阈值逻辑：当 top-K 未满时按原始 score_threshold 剪枝；满了以后按当前第 K 名分数剪枝。
 */
...
void BBS3D::localize() {
  ...
  std::vector<DiscreteTransformation<double>> top_transforms;

  auto pruning_score = [&]() {
    if (static_cast<int>(top_transforms.size()) < top_k_) {
      return score_threshold;
    }
    return top_transforms.back().score;
  };

  auto push_top_leaf = [&](const DiscreteTransformation<double>& leaf) {
    if (leaf.score < score_threshold) {
      return;
    }
    top_transforms.push_back(leaf);
    std::sort(top_transforms.begin(), top_transforms.end(), [](const auto& lhs, const auto& rhs) {
      return lhs.score > rhs.score;
    });
    if (static_cast<int>(top_transforms.size()) > top_k_) {
      top_transforms.resize(top_k_);
    }
  };
...
#pragma omp parallel for num_threads(num_threads_)
  ...
  // Not localized
  if (top_transforms.empty() || has_timed_out_) {
    has_localized_ = false;
    return;
  }

  double min_res = voxelmaps_ptr_->get_min_res();
  global_pose_ = top_transforms.front().create_matrix(min_res, ang_info_vec[0].rpy_res, ang_info_vec[0].min_rpy);
  best_score_ = top_transforms.front().score;
  for (auto top_trans : top_transforms) {
    top_poses_.push_back(top_trans.create_matrix(min_res, ang_info_vec[0].rpy_res, ang_info_vec[0].min_rpy));
    top_scores_.push_back(top_trans.score);
  }
  has_timed_out_ = false;
  has_localized_ = true;
}
