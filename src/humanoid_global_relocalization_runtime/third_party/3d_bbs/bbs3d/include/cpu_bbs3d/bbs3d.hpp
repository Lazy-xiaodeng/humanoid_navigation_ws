 ...
 *
 * 文件作用：
 *   1. 这是 vendored KOKIAOKI/3d_bbs 的 CPU 版 3D-BBS 后端接口。
 *   2. humanoid_global_relocalization_runtime 直接通过该接口构建地图 voxel、设置搜索范围并执行全局搜索。
 *   3. 本包在上游接口基础上增加 top-K 候选导出，用于离线 CSV 和 RViz/debug 检查。
 *
 * 修改边界：
 *   - 保持原始分支定界算法主体不变。
 *   - 新增 set_top_k/get_top_poses/get_top_scores，只暴露搜索过程中已经到达 leaf 的高分候选。
 */
...
public:
  ...
  void set_top_k(int top_k) { top_k_ = std::max(1, top_k); }
  ...
  std::vector<Eigen::Matrix4d> get_top_poses() const { return top_poses_; }

  std::vector<int> get_top_scores() const { return top_scores_; }
...
private:
  ...
  int best_score_;
  int top_k_;
  std::vector<Eigen::Matrix4d> top_poses_;
