...
ensure_runtime_environment() {
    local user_id
    user_id="$(id -u ubuntu 2>/dev/null || id -u)"
...
cleanup_old_speech_processes() {
    ...
    if command -v timeout >/dev/null 2>&1; then
        timeout 5s pkill -f "/home/ubuntu/asr-client/listening_sound.py" 2>/dev/null || true
        timeout 5s pkill -f "asr-client-master.1.0.jar" 2>/dev/null || true
    else
        pkill -f "/home/ubuntu/asr-client/listening_sound.py" 2>/dev/null || true
...
start_speech() {
    ...
    local asr_dir="/home/ubuntu/asr-client"
